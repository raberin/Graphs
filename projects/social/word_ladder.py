...
def get_neighbors(word):
    ...
    # Loop through each letter in string_word
    for i in range(len(string_word)):
        # Loop through every letter in alphabet
        for letter in ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']:
            # Modify a copy of string_word at a specific index/letter
            temp_word = list(string_word)
            temp_word[i] = letter
            # Join this new temp word together with a changed letter from original word
            w = "".join(temp_word)
            # Check if this word is in the word_set/dictionary
            # If so... append to neighbors
